...
const pluginModule = {
    ...
    init(api) {
        ...
        unsubscribers.push(api.events.on('room:gameStart', () => {
            api.logger.debug('[websocket] event: room:gameStart');
            instance.broadcastRooms();
        }));
        unsubscribers.push(api.events.on('room:gameEnd', () => {
            api.logger.debug('[websocket] event: room:gameEnd');
            instance.broadcastRooms();
        }));
        unsubscribers.push(api.events.on('player:connect', () => {
            api.logger.debug('[websocket] event: player:connect');
            instance.broadcastRooms();
        }));
        unsubscribers.push(api.events.on('player:disconnect', () => {
            api.logger.debug('[websocket] event: player:disconnect');
            instance.broadcastRooms();
        }));
        unsubscribers.push(api.events.on('chat:message', () => {
